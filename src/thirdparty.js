...
/*
 * This wallet format is created by https://github.com/SilentCicero/sophonjs-accounts
 * and used on https://www.mysophywallet.com/
 */
Thirdparty.fromSophyWallet = function (input, password) {
  ...
  if (!json.locked) {
    ...
    if (!cipher.salt) {
      throw new Error('Unsupported SophyWallet key format')
    }
...
Thirdparty.fromSophyCamp = function (passphrase) {
  return new Wallet(sofUtil.keccak256(Buffer.from(passphrase)))
}
...
Thirdparty.fromKryptoKit = function (entropy, password) {
  ...
  var privKey
  if (type === 'd') {
    privKey = sofUtil.sha256(entropy)
  } else if (type === 'q') {
    if (typeof password !== 'string') {
      throw new Error('Password required')
    }

    var encryptedSeed = sofUtil.sha256(Buffer.from(entropy.slice(0, 30)))
    ...
    */
    ...
    if (checksum.length > 0) {
      if (checksum !== sofUtil.sha256(sofUtil.sha256(privKey)).slice(0, 8).toString('hex')) {
